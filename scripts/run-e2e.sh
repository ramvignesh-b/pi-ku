...
# This cleans up the django backend process and, for local runs, the containers.
cleanup() {
    echo "Cleaning up..."
...
if [ "$CI" = "true" ]; then
    # In CI, Postgres and Mailpit are provided as native workflow `services:`
    # containers - already up and healthy before this script runs, with
    # .env.e2e already pointed at them by the workflow. No container engine
    # needed here at all.
    echo "Running in CI: using services provisioned by the workflow (DB_HOST=$DB_HOST, EMAIL_HOST=$EMAIL_HOST)."
else
...
if [ $MODE = "docker" ]; then
    if [ -z "$CONTAINER_BIN" ]; then
        echo "--docker needs a container engine, which isn't used in CI runs."
        exit 1
    fi
